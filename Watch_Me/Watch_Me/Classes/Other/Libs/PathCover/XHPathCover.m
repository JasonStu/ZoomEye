...
- (void)setOffsetY:(CGFloat)y {
    ...
    if(y < 0) {
        ...
        if(y < bframe.size.height) {
            CGPoint center =  _bannerImageView.center;
            center.y = bannerSuper.frame.size.height/2 + 0.3 * y;
            _bannerImageView.center = center;
        }
    }
...
- (void)_setup {
    ...
    _avatarButton.layer.cornerRadius = 33;
    
    _userNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(93, 0, 207, 34)];
    _userNameLabel.textColor = [UIColor warningColor];
    ...
    UIView  *userLabel =[[UIView alloc] initWithFrame:CGRectMake(0, 42, self.showView.frame.size.width, 24)];
    userLabel.backgroundColor = [UIColor black25PercentColor];
    userLabel.alpha = 0.1;
...
//    _birthdayLabel.shadowOffset = CGSizeMake(0, 1);
    _birthdayLabel.font = [UIFont systemFontOfSize:13.0f];
    _birthdayLabel.numberOfLines = 0;

    [_showView addSubview:userLabel];
